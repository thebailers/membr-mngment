...
import ClassesWrapper from "../../classes/classes-wrapper.component";
...
describe("the specific class component", () => {
  ...
  it("prevents member signing in if too long before class", async () => {
    jest.useFakeTimers();
    // jest.setSystemTime(new Date("Sun Jan 22 2023 18:15:00 GMT+0000"));
    setup("/classes/monday/1830");
  ...
  it("allows the user to sign in if the current time is close to the class time", async () => {
    ...
    setup("/classes/monday/1830");
    act(() => jest.advanceTimersByTime(2000));
    expect(await screen.findByText(/Class sign in/i)).toBeInTheDocument();
    jest.useRealTimers();
  });

  it("displays a loading message while data & positive class sign in status loads", async () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date("Mon Jan 23 2023 18:15:00 GMT+0000"));
    setup("/classes/monday/1830");
  ...
  it("displays a loading message while data & negative class sign in status loads", async () => {
    jest.useFakeTimers();
